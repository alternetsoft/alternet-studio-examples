...
namespace Alternet.Editor.CustomizeDialog.AlternetUI
{
    ...
    public class SyntaxSettings : PersistentSettings, ISyntaxSettings
    {
        ...
        protected virtual void ApplyEventData(IKeyList applyTo, IKeyDataList getFrom)
        {
            ...
            List<IKeyData> applyToList = [];
            List<IKeyData> getFromList = [];

            foreach (var keyData in getFrom)
                getFromList.Add(keyData);

            List<IKeyData> itemsToRemove = [];

            for (int i = applyTo.EventData.Length - 1; i >= 0; i--)
            {
                var oldItem = applyTo.EventData[i];
                var newData = FindKeyData(getFrom, oldItem);
                if (newData is null)
                {
            ...
            {
                applyTo.Remove(itemToRemove.Keys, itemToRemove.State);
            }

            foreach (var keyData in applyTo.EventData)
                applyToList.Add(keyData);

            for (int i = applyToList.Count - 1; i >= 0; i--)
            {
                var newData = FindKeyData(getFrom, applyToList[i]);
                if (newData != null)
                {
                    applyToList.RemoveAt(i);
                    getFromList.Remove(newData);
                }
                else
                {
            ...
            foreach (var keyData in applyToList)
            {
                var kd = FindBestCandidate(getFromList, keyData);
                if(kd is not null)
            ...
            foreach (var key in eventsToUpdate.Keys)
            {
                ...
                if (keyData != null)
                {
                    applyTo.Remove(key.Keys, key.State);
                    if (key.ActionEx != null)
                        applyTo.Add(keyData.Keys, key.ActionEx, key.Param, key.State, key.LeaveState);
                    else
                        applyTo.Add(keyData.Keys, key.Action, key.State, key.LeaveState);
                }
            }
        }

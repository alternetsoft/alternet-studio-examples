...
namespace RSyntaxParsing
{
    public class ViewModel : INotifyPropertyChanged
    {
        private RParserEmbedded rParserEmbedded = new RParserEmbedded();
        ...
        public ViewModel(Window window, TextEditor edit)
        {
            ...
            FileInfo fileInfo = new FileInfo(dir + @"Resources\Editor\Text\r.txt");
            if (fileInfo.Exists)
            {
            ...
            openFileDialog.Filter = "R files (*.r)|*.r|All files (*.*)|*.*";

            edit.Lexer = rParserEmbedded;
        ...
        {
            if (RParserEmbedded.IsServerDeployed())
                return;

            var progressDialog = new ProgressDialog()
            {
                ShowInTaskbar = true,
                Title = "LSP R Parser Demo",
                Message = "Please wait until the R LSP server files are extracted...",
            };
            ...
            progressDialog.Loaded += async (_, __) =>
            {
                await Task.Run(() =>
                {
                    RParserEmbedded.DeployServer(progressDialog.Progress);

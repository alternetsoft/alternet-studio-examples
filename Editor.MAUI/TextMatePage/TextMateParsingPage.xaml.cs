...
{
    internal static string increasePattern = "^((?!//).)*(\\{([^}\"'`/]*|(\\t|[ ])*//.*)|\\([^)\"'`/]*|\\[[^\\]\"'`/]*)$";
    internal static string decreasePattern = "^((?!.*?\\/\\*).*\\*/)?\\s*[\\)\\}\\]].*$";
    internal static string unindentPattern = "^(\\t|[ ])*[ ]\\*[^/]*\\*/\\s*$|^(\\t|[ ])*[ ]\\*/\\s*$|^(\\t|[ ])*[ ]\\*([ ]([^\\*]|\\*(?!/))*)?$";

    internal string SampleFileNameNoExt = "embres:AllQuickStarts.Content.TextMate.";

    private readonly List<string> langList = new ();
    private readonly List<string> themeList = new ();
    ...
    private LanguageInfo[] langItems =
    {
            ...
            new LanguageInfo(".cshtml", "*.cshtml", "sample.cshtml2", "Razor"),
    ...
    private void LanguagesPicker_SelectedIndexChanged(object? sender, EventArgs e)
    {
        ...
        if (LanguagesPicker.SelectedIndex >= 0)
        {
            ...
            if (fileName != string.Empty)
            {
                ...
                syntaxEdit1.Source.FileName = Path.ChangeExtension(fileName, ext);
                ReparseText();
            }

...
namespace AllQuickStarts
{
    public class SQLWrapper : SyntaxParser
    {
        ...
        public override int Outline(IList<Alternet.Common.IRange> ranges)
        {
        ...
        public override int GetSyntaxErrors(IList<ISyntaxError> errors)
        {
            if (scriptErrors != null)
            {
                ...
                {
                    var syntaxError = new SyntaxError(new System.Drawing.Point(error.Column - 1, error.Line - 1), string.Empty, error.Message);
                    var errorToken = FindErrorToken(error);
                    syntaxError.Range.EndPoint = (errorToken != null) && (!string.IsNullOrEmpty(errorToken.Text)) ? new System.Drawing.Point(errorToken.Column + errorToken.Text.Length - 1, errorToken.Line - 1) : new System.Drawing.Point(error.Column, error.Line - 1);
        ...
        {
            var repository = CompletionRepository as SqlWrapRepository;
            int i = 0;
            int len = s.Length;
            while (i < len)
            {
                if (colorData[i].Data == 2 + 1)
                {
                    ...
                    var obj = repository?.Reswords[word.ToUpper()];
                    if (obj != null)
                    {
                        var rword = obj.ToString()?.ToUpper();
                        if (rword != word)
        ...
        protected virtual void Outline(IList<Alternet.Common.IRange> ranges, IList<TSqlStatement> statements, int level)
        {
            foreach (TSqlStatement stat in statements)
            {
                if (IsCollapsibleStatement(stat))
                {
                    var startToken = (stat.FirstTokenIndex >= 0 && stat.FirstTokenIndex < stat.ScriptTokenStream.Count) ? stat.ScriptTokenStream[stat.FirstTokenIndex] : null;
                    var endToken = (stat.LastTokenIndex >= 0 && stat.LastTokenIndex < stat.ScriptTokenStream.Count) ? stat.ScriptTokenStream[stat.LastTokenIndex] : null;
                    if ((startToken != null) && (endToken != null) && (endToken.Line > startToken.Line))
                    {
                        ranges.Add(
                            new OutlineRange(
                                new System.Drawing.Point(startToken.Column - 1, startToken.Line - 1),
                                !string.IsNullOrEmpty(endToken.Text) ? new System.Drawing.Point(endToken.Column - 1 + endToken.Text.Length, endToken.Line - 1) : new System.Drawing.Point(endToken.Column - 1, endToken.Line - 1),
                                level,
        ...
        private TSqlParserToken? GetPrevToken(System.Drawing.Point position)
        {
            foreach (TSqlParserToken token in scriptTokenStream)
            {
        ...
        private TSqlParserToken? GetToken(System.Drawing.Point position)
        {
            foreach (TSqlParserToken token in scriptTokenStream)
            {
                if (token.Line == position.Y + 1)
        ...
        {
            var result = scriptTokenStream.Where(tok => tok.Line == y + 1).Where(tok => tok.Column <= x + 1).Where(tok => IsKeywordOrIdentifier(tok)).LastOrDefault();
            return result != null ? result.Text : string.Empty;
        }

        private string GetCurWord(int x, int y)
        {
            var result = scriptTokenStream.Where(tok => tok.Line == y + 1).Where(tok => tok.Column <= x + 1).Where(tok => tok.Column + tok.Text.Length > x).Where(tok => IsKeywordOrIdentifier(tok)).LastOrDefault();
            return result != null ? result.Text : string.Empty;
        }

        private string GetNextWord(int x, int y)
        {
            var result = scriptTokenStream.Where(tok => tok.Line == y + 1).Where(tok => tok.Column >= x + 1).Where(tok => IsKeywordOrIdentifier(tok)).FirstOrDefault();
            return result != null ? result.Text : string.Empty;
        }

        private string GetDelimiter(int x, int y)
        {
            var result = scriptTokenStream.Where(tok => tok.Line == y + 1).Where(tok => tok.Column <= x + 1).Where(tok => IsDelimiter(tok)).LastOrDefault();
            return result != null ? result.Text : string.Empty;
        }
        ...
        private void FillMembers(CodeCompletionArgs e, System.Drawing.Point position, string ident, string word, bool partial)
        {
            var repository = CompletionRepository as SqlWrapRepository;
            ...
            else
            {
                if (fillFields)
                {
                    if (repository is not null && repository.FindTable(tableName) && fillFields)
                    {
                        e.Provider = new SqlWrapListMembers();
    ...
    public class SQLTokenizer : IUpdate
    {
        ...
        public void ParseText(int line, ref StringItemInfo[] colorData)
        {
            ...
            if (tokens == null)
                return;
            var lineTokens = tokens.GetTokens(line, false);
        ...
        {
            var lineTokens = tokens.GetTokens(tok.Line - 1, true);
...
#pragma warning restore SA1402 // File may only contain a single type
    {
        ...
        private IList<LineToken>? CopyNextTokens(int index, int start)
        {
            ...
            foreach (LineToken token in thisIndex)
            {
        ...
        {
            IList<LineToken>? result = tokens != null
                ? tokens : (this[index] != null ? new List<LineToken>() : null);

            var thisIndex = this[index];

            if (result != null && thisIndex != null)
            {
                foreach (LineToken token in thisIndex)
                {
                    if (token.Start >= start)

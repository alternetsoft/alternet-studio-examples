...
namespace AlternetStudio.TypeScript.Wpf.Demo
{
    public partial class MainWindow
    {
        ...
        {
            openFileDialog.Filter = "Typescript files (*.ts)|*.ts|Javascript files (*.js)|*.js|Project Files (*.json)|*.json|Any files (*.*) |*.*";
            saveFileDialog.Filter = "Typescript files (*.ts)|*.ts|Javascript files (*.js)|*.js|Project Files (*.json)|*.json|Any files (*.*) |*.*";
            ...
            if (new DirectoryInfo(startupDirectory).Exists)
            {
                ...
                var projectFile = Path.GetFullPath(Path.Combine(startupDirectory, @"Debugger.TypeScript\TS\MultipleFiles\project.json"));

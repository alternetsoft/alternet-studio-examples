...
namespace Alternet.Scripter.Integration.AlternetUI
{
    public class DebuggerIntegrationPanelCSharp : BaseDebuggerIntegrationPanel
    {
        ...
        public DebuggerIntegrationPanelCSharp()
        {
            ...
            OpenProjectDialog.Filter = "Project files (*.csproj; *.vbproj)|*.csproj;*.vbproj|All files (*.*)|*.*";
        }
        ...
        public virtual void OpenProject(string? projectFilePath)
        {
            ...
            proj.Load(projectFilePath);
            (ScriptRun?.ScriptSource as IScriptSource)?.FromScriptProject(proj.ProjectFileName);
            var extension = proj.ProjectExtension;
            OpenProject(extension, proj);

            var codeFiles = proj.Files.Where(
                x => Path.GetExtension(x) == ".cs" || Path.GetExtension(x) == ".vb").ToList();

            if (codeFiles.Count > 0)
            {
                CodeEditContainer.TryActivateEditor(GetFirstFile(codeFiles, proj.DefaultExtension));
            }

            var references = proj.References.Concat(proj.AutoReferences).Select(
                x => x.FullName).Concat(
                proj.FrameworkReferences.SelectMany(x => x.Assemblies).Select(

...
namespace DebuggerIntegration.Python
{
    public partial class Form1 : Window
    {
        ...
        public Form1()
        {
            ...
            debuggerPanels.VisiblePanels &= ~DebuggerPanelKinds.Threads;
            debuggerPanels.Debugger = debugger;
            ...
            uiController.ExceptionOccurred += (s, evt) =>
            {
                var e = evt.Value;

                ScriptCompilationDiagnostic error = new()
                {
                    Code = e.Exception?.ExceptionType,
                    Message = e.Exception?.Message,
                    FileName = e.Position?.File,
                    Line = e.Position?.StartLine ?? 0,
                    Column = e.Position?.StartColumn ?? 0,
                    Kind = ScriptCompilationDiagnosticKind.Error,
                };

                debuggerPanels.Errors?.AddCompilerError(error);
            };
